...
from scripts.output_writer import (
    ...
    guard_nba_empty_output, write_nba_abort_log, write_nba_features, write_nba_results, DATA_DIR,
)
...
def run_nba():
    ...
    if not scoreboard_games and not future_games:
        ...
        if not calendar_offseason and guard_nba_empty_output(existing_path):
            ...
            write_nba_abort_log(
                reason=msg,
                now_utc=now_utc,
                counts={"scoreboard_games": len(scoreboard_games), "future_games": len(future_games)},
            )
            return
        if guard_nba_empty_output(existing_path):
            msg = "Both scoreboard and future schedule fetches returned 0 games — aborting to preserve existing nba_predictions.json"
            log.error(msg)
            write_nba_abort_log(
                reason=msg,
                now_utc=now_utc,
    ...
    if len(all_games) == 0 and len(historical_games) >= 200 and not is_offseason:
        msg = (
            f"Historical fetch OK ({len(historical_games)} games) but 0 current/future "
            f"games found during NBA season — aborting to preserve existing nba_predictions.json"
        )
        log.error(msg)
        write_nba_abort_log(
            reason=msg,
